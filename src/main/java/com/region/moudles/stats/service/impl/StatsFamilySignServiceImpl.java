...
import com.region.moudles.stats.dao.StatsFamilySignDao;
...
@Service("statsFamilySignService")
public class StatsFamilySignServiceImpl implements StatsFamilySignService {

	@Inject
	private StatsFamilySignDao familySignDao;
	
	/**
	 * 	获取家医菜单中家庭签约统计签约、履约人次、履约人数、解约（echarts）
	 */
	@Override
	public Map<String, Object> getJYTimesStats(StatsPlatResultModel info)throws Exception {
		...
		//家医统计
		List<ResultModel> ylList = familySignDao.getYLCharges(info);
		...
		for(int i = 0; i < ylList.size(); i++) {
			if(ylList.size() == (i+2)) {
				...
				sumList.add(jc);
				ResultModel ss = new ResultModel();
				ss.setName("年度解约人次");
	 ...
	 */
	@Override
	public List<ResultModel> getQXJYTimesStats(String incomeType, String orgCode)throws Exception {
		
		List<ResultModel> list = familySignDao.getQXYLChargeStats(incomeType, orgCode);
		
		if(list != null && list.size() > 0) {
			for(ResultModel info : list) {
				String name = info.getName().replaceAll("卫健局", "").replaceAll("云南省曲靖市马龙县", "").replaceAll("街道办事处", "");
				info.setName(name);
			}
			//list中数据降序排列
			list.sort(new Comparator<ResultModel>() {
				@Override
	...
	public void getJGLBTimesPage(StatsPlatResultModel info, PageModel page)throws Exception {
		...
		this.converDateInfo(info);
		familySignDao.getJGLBSRCharges(info, page);
	}
	...
	public List<ResultModel> getJYItemPropStats(StatsPlatResultModel info)throws Exception {
		this.converDateInfo(info);
		return familySignDao.getYLItemTypeStats(info);
	}
	...
	public Map<String, Object> getJYItemTimesStats(StatsPlatResultModel info)throws Exception {
		...
		List<ResultModel> list = familySignDao.getYLItemChargeStats(info);
