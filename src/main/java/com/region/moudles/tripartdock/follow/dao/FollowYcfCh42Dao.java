...
import com.region.moudles.tripartdock.follow.dao.repository.FollowYcfCh42Repository;
...
import com.region.moudles.tripartdock.follow.dto.Yfch42Dto;
...
@Named
public class FollowYcfCh42Dao extends BaseDao<FollowYcfCh42Repository, FollowYcfCh42> {

    public List<Yfch42Dto> getAll(RequestDto request) {
        StringBuilder sql = new StringBuilder();
        sql.append("select * from (select t.*,ROWNUM rn from( ");
        sql.append(" select b.id,b.jmid,b.dah,b.sfzh,b.sfrq,jb.hzmc yfxm,o.id org_id,o.org_name,b.sfys as sfys_name");
        sql.append(" from blt_ycfch42txxb b");
        ...
        List<Yfch42Dto> list = queryList(sql.toString(), request, Yfch42Dto.class);
        return list;
    }

    public int getCount(RequestDto request) {
        StringBuilder sql = new StringBuilder();
        sql.append("select b.id from blt_ycfch42txxb b join blt_jbxxjlb jb on b.jmid = jb.id join sys_org o on o.id = jb.jdjg ");
        sql.append(" where 1=1 ");
        if ( StringUtils.isNotBlank(request.getJmid())) {
            sql.append("  and b.jmid = :jmid ");
        }
    ...
    public int getSFCountByJmId(String jmId) {
        StringBuilder sql = new StringBuilder();
        sql.append("select a.id from blt_ycfch42txxb a ");
