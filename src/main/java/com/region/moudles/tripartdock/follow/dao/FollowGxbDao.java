...
import com.region.moudles.tripartdock.follow.dao.repository.FollowGxbRepository;
...
@Named
public class FollowGxbDao extends BaseDao<FollowGxbRepository, FollowGxb> {

    public List<GxbDto> getAll(RequestDto request) {
        StringBuilder sql = new StringBuilder();
        sql.append("select * from (select t.*,ROWNUM rn from( ");
        sql.append(" select b.id,b.sfrq,b.sflx sffs,b.sfys,jb.hzmc,o.id org_id,o.org_name,b.sfys as sfys_name");
        sql.append(" from blt_mxgxbsfb b");
        ...
        List<GxbDto> list = queryList(sql.toString(), request, GxbDto.class);
        return list;
    }

    public int getCount(RequestDto request) {
        StringBuilder sql = new StringBuilder();
        sql.append("select b.id from blt_mxgxbsfb b join blt_jbxxjlb jb on b.jmid = jb.id join sys_org o on o.id = jb.jdjg ");
        sql.append(" where 1=1 ");
        if ( StringUtils.isNotBlank(request.getJmid())) {
            sql.append("  and b.jmid = :jmid ");
        }
    ...
    public int getSFCountByJmId(String jmId) {
        StringBuilder sql = new StringBuilder();
        sql.append("select a.id from blt_mxgxbsfb a ");
