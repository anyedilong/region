...
import com.region.moudles.tripartdock.follow.dao.repository.FollowYcfCqRepository;
...
import com.region.moudles.tripartdock.follow.dto.YfcqDto;
...
@Named
public class FollowYcfCqDao extends BaseDao<FollowYcfCqRepository, FollowYcfCq> {

    public List<YfcqDto> getAll(RequestDto request) {
        StringBuilder sql = new StringBuilder();
        sql.append("select * from (select t.*,ROWNUM rn from( ");
        sql.append(" select b.id,b.jmid,b.sfzh,b.sfrq,b.sfcs stage,jb.hzmc yfxm,o.id org_id,o.org_name,b.sfys as sfys_name");
        sql.append(" from blt_yf2_5csfb b");
        ...
        List<YfcqDto> list = queryList(sql.toString(), request, YfcqDto.class);
        return list;
    }

    public int getCount(RequestDto request) {
        StringBuilder sql = new StringBuilder();
        sql.append("select b.id from blt_yf2_5csfb b join blt_jbxxjlb jb on b.jmid = jb.id join sys_org o on o.id = jb.jdjg ");
        sql.append(" where 1=1 ");
        if ( StringUtils.isNotBlank(request.getJmid())) {
            sql.append("  and b.jmid = :jmid ");
        }
    ...
    public int getSFCountByJmId(String jmId) {
        StringBuilder sql = new StringBuilder();
        sql.append("select a.id from blt_yf2_5csfb a ");
